...
//
// Terminates the execution of the UThread passed as paramether and continues
// the execution of the RunningThread.
//
...
//
// Terminates the execution of the UThread passed as paramether and continues
// the execution of the next ready Thread.
//
...
//
// Returns the State (Running, Ready, Blocked) of the UThread handle passed as argument 
//
...
BOOL UtAlive(HANDLE thread){
	...
	PLIST_ENTRY start=&AliveQueue;
	for(;;){
		if(start==&ut->AliveLink)
			return TRUE;		
		start=start->Flink;
		if(start==&AliveQueue)
			return FALSE;
...
VOID UtActivate (HANDLE ThreadHandle) {
	if(((PUTHREAD)ThreadHandle)->ToTerminate) 
		UtExitAux2(ThreadHandle);
